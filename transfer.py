...
def transfer(args):
  ...
  with tf.Graph().as_default(), tf.device('/cpu:0'):
      ...
      for i in xrange(args.num_gpus):
        with tf.device('/gpu:%d' % i):
          with tf.name_scope('Tower_%d' % i) as scope:
            ...
            # all towers.
            if args.finetune:
              logits = arch.get_model(images, wd, True, args, transferMode=True)
            else:
              logits = arch.get_model(images, wd, False, args, transferMode=False)
            # Top-1 accuracy
           ...
           # Calculate the gradients for the batch of data on this CIFAR tower.
            if args.finetune:
              ...
              grads = opt.compute_gradients(total_loss,var_list= tf.get_collection(tf.GraphKeys.VARIABLES, scope='transfer_output'))
    ...
    for epoch in xrange(1, args.num_epochs):
      # update epoch_number
...
def main():  # pylint: disable=unused-argument
    ...
    parser.add_argument('--path_prefix' , default= '/lscratch/datasets/serengeti/resized/', action= 'store', help= 'The prefix address for images')
    parser.add_argument('--delimiter', default= ',', action = 'store', help= 'Delimiter of the labels')
    parser.add_argument('--data_info'  , default= 'silver_info_train.csv', action='store', help= 'File containing the addresses and labels of training images')
